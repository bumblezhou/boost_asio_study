...
public:
  ...
  request(command_type cmd, const boost::asio::ip::tcp::endpoint& endpoint, const std::string& user_id)
    : version_(version),
      command_(cmd),
      user_id_(user_id),
      null_byte_(0) {
    ...
    // Covert port number to network byte order.
...
public:
  ...
  std::array<boost::asio::mutable_buffer, 5> buffer() {
    return {
      {
        boost::asio::buffer(&null_byte_, 1),
