...
void forwardRequest(boost::asio::ssl::stream<tcp::socket>& ssl_socket) {
  ...
  while (true) {
    size_t bytes_transferred = boost::asio::read(destination_socket, response_buffer, boost::asio::transfer_at_least(1), error);
...
int main() {
  ...
  ssl_context.use_private_key_file("/home/jack/Desktop/Tmp/certs/jackzhou.me.key", boost::asio::ssl::context::pem);
  ssl_context.use_certificate_chain_file("/home/jack/Desktop/Tmp/certs/jackzhou.me.crt");
